...
Firstly, we treat Y1 blocks and Y2 blocks as two block sequence, which means each block has a unique index. The treatment of Y blocks as seqeunces indicate the first case leading to tag collision.
...
Secondly we treat Y1 blocks and Y2 blocks are two sets allowing identical elements. We give the definition of set equality below:
\begin{defination}
If the a input block of rotate shifting stage X$_1$[i ]from X1 sequence is identical to the block X$_2$[i] with same index from X2 sequence, the following two properties exist:
\end{defination}
If Y1 and Y2 sets are set-level identically, then T1 is identical to T2.

We find that even none of element in X set is formed with If any element in identical sub-sets of two X sets can be formed by rotate shifting another element in the same sub-set, the corresponding two Y sub-sets, Y$_{sub}$1 and Y$_{sub}$2, have high probability to be SIE.
